...
if not exist "%AppData%\Apple Computer\iTunes\iPhone Software Updates\iPhone13,3_*.*_*_Restore.ipsw" exit
...
for /F Tokens^=1-4^ Delims^=^ EOL^= %%T in ('dir /B /A:-D^
 "%AppData%\Apple Computer\iTunes\iPhone Software Updates\iPhone13,3_*.*_*_Restore.ipsw"'
) do (
