...
if not exist "%AppData%\Apple Computer\iTunes\iPad Software Updates\iPad_Pro_HFR_*.*_*_Restore.ipsw" exit
...
for /F Tokens^=1-4^ Delims^=^ EOL^= %%T in ('dir /B /A:-D^
 "%AppData%\Apple Computer\iTunes\iPad Software Updates\iPad_Pro_HFR_*.*_*_Restore.ipsw"'
) do (
